...
The status-bar icon is a template image set in `assets/menubar/` (idle waveform, recording dot, processing arc, ...), generated by `assets/generate_menubar.py` (Pillow, deterministic). `branding.split_menubar_title()` maps the emoji state tokens in `MenuBarIcons` to these assets inside `app._set_title`; when the assets are missing (e.g. a bare package install without the repo checkout) the emoji titles themselves are the fallback, so nothing breaks. `recording` and `processing` also ship numbered `<state>.frameN.png` files that a `rumps.Timer` cycles for the pulse/spinner animation — the timer runs only while one of those states is active. Regenerate after design changes with:
